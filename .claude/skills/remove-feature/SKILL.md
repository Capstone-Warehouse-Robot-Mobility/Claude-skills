...
Before defining the deletion boundary, identify which app(s) and package(s) the removed feature lives in (and which OTHER stacks may have callers) by reading the user's prompt for explicit paths/feature names and mapping to the monorepo layout — see `.claude/stack-map.md` for the full path → reference-skill mapping.

For each touched stack, name the framework-reference skill(s) (e.g., `heroui-native`, `elysiajs`, `vercel-react-best-practices`) you'll consult when tracing references and verifying nothing downstream still depends on what's being removed. **Multi-stack removals** (deleting a server endpoint that web and native both call) consult refs for all stacks that might hold references — the deletion isn't complete until every caller is updated.

The refs are reference docs (patterns, API surface, conventions) not workflows; consult via `Skill(skill="<name>")` or `Read` the file at `.claude/skills/<name>/SKILL.md`. Skip refs only when the deletion is clearly local (a single helper used by one file).

**Announce the detected stack(s) and refs** in your initial response so the user can correct the assumption before the boundary is defined.
...
If a check doesn't exist in this project (no test suite, no type-checker, no linter), note the gap in the MR/removal report rather than skipping silently — coverage holes are themselves part of the removal's risk surface.
...
- `add-migration` — when the removal changes schema, drops/stops-writing columns, removes enum values from persisted rows, changes indexes/constraints, or requires a backfill/purge/historical-data decision.
...
- `sync-docs` — recommend or invoke when existing docs, API docs, README, `.env.example`, setup docs, or runbooks mention the removed feature. This skill updates existing docs only; it does not create new files.
...
- **NEVER expand scope mid-removal to refactor adjacent code**
  **Instead:** File a follow-up task; keep the deletion MR scoped to the deletion.
  **Why:** A destructive MR mixed with refactor changes is impossible to review and impossible to bisect when something breaks downstream — the rollback options collapse to all-or-nothing.
...
- **NEVER paper over a Phase 6 check failure with a stub, `any` cast, or skipped test**
  ...
  **Why:** A stub left behind in a deletion MR is itself dead code — the very thing this skill exists to prevent.
...
After removal, surrounding code often loses a reason to exist as a separate abstraction (lone-caller helpers, single-branch conditionals, parameters only used by the deleted path). Run `simplify` against the diff to collapse the leftovers.
