...
description: Pre-MR gauntlet — run the project's typecheck, linter, formatter, and test suite against the branch's diff vs. the base, plus a residue sweep for `console.log`, `.only` / `.skip`, debugger statements, leftover TODOs in changed files, dropped lockfiles, and unintended large/binary additions. Reports each gate's pass/fail with the exact reproduction command. Auto-fixes only the format-and-lint-fix gate when it has a known auto-fix; everything else is reported, never silently fixed. Trigger phrases — "pre-mr check", "ready to MR?", "is this MRable", "pre-flight this branch", "gauntlet", "/check-mr-readiness", "lint+test+types before MR". Skip for — branches with no commits beyond base, doc-only or comment-only diffs (still useful but optional), repos with no CI conventions to enforce.
---
...
A pre-flight gate for code that is about to go on a merge request. Each check has one exit condition: **green** or **report-and-stop**. Do not run the next check if the previous fails — surface the failure first.
...
| lint | script `lint` / `lint:check`; else local config presence (`eslint.config.*`, `.eslintrc*`, `biome.json`, `.oxlintrc.json`) → `npx eslint <changed-files>` / `npx @biomejs/biome lint <changed-files>` / `npx oxlint <changed-files>` |
| format | script `format:check` / `prettier:check`; else config presence (`.prettierrc*`, `biome.json`, `dprint.json`, `oxfmt.toml`) → `npx prettier --check <changed-files>` / `npx @biomejs/biome format <changed-files>` / `npx dprint check <changed-files>` / `npx oxfmt --check <changed-files>` |
...
Lint matchers recognized: `eslint`, `biome` (Biome's `lint` subcommand), `oxlint` (Rust-based; invoked as `oxlint` or via `pnpm oxlint`). Formatter matchers recognized: `prettier`, `biome` (`format` subcommand), `dprint`, `oxfmt` (Rust-based; `oxfmt --check` to verify, `oxfmt --write` to auto-fix).
...
If `format` failed and a write-mode command exists for the detected formatter, offer to run it:

| Formatter detected | Write command |
|---|---|
| prettier | `npx prettier --write <changed-files>` (or project script `format` / `prettier:write`) |
| biome | `npx @biomejs/biome format --write <changed-files>` (or script `format`) |
| dprint | `npx dprint fmt <changed-files>` |
| oxfmt | `npx oxfmt --write <changed-files>` (or `pnpm oxfmt --write`) |

For `lint`, only offer auto-fix when the detected linter is invoked with an auto-fix flag that the project already wires up (e.g., `pnpm lint:fix`, `npx eslint --fix`, `npx @biomejs/biome lint --apply`, `npx oxlint --fix`). The auto-fix-lint gate's logic stays: if the project's lint command auto-fixes or its format command writes, the gate runs it; otherwise it reports and asks.
...
- **NEVER auto-fix lint or test failures.**
  **Instead:** report the failure with the exact command to reproduce. Auto-fix only format issues (or lint-fix when the project's lint command already implies it), and only after user `y`.
...
- **NEVER invent commands the project doesn't define.**
  **Instead:** if no `lint` script and no eslint/biome/oxlint config exists, mark lint `n/a` and skip. Same for format: require prettier/biome/dprint/oxfmt config before invoking.
  **Why:** running an arbitrary linter or formatter on a project that doesn't use one produces a flood of irrelevant errors and trains the user to ignore the gauntlet.
...
- **NEVER push or open an MR from this skill.**
  **Instead:** report the verdict and stop. The user invokes the MR-creation skill separately when they're ready.
  **Why:** this is a gate, not a workflow step. Coupling it to push/MR conflates "checked" with "ready", and a developer often wants to see the report before deciding.
