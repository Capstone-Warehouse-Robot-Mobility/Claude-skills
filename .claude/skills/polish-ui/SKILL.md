...
Before walking the checklist, identify whether the touched UI is in `apps/web/` (TanStack Start / shadcn) or `apps/native/` (Expo / HeroUI Native) — see `.claude/stack-map.md` for the full path → reference-skill mapping. The two stacks have different idioms (mouse vs touch, `Kbd` vs no-keyboard, focus management, etc.); some checklist items apply to only one.

For the touched stack, name the framework-reference skill(s) (e.g., `heroui-native`, `building-native-ui`, `shadcn`, `vercel-react-best-practices`) you'll consult while applying the checklist. **Cross-platform UI** (a component shared between web and native via `packages/ui` or duplicated parity surfaces) consults refs for both — and the checklist item that doesn't translate gets explicitly skipped with a one-line reason.

The refs are reference docs (patterns, API surface, conventions). Consult via `Skill(skill="<name>")` or `Read` the file at `.claude/skills/<name>/SKILL.md`.

**Announce the detected stack(s) and refs** before walking the checklist so the user can correct the assumption.
