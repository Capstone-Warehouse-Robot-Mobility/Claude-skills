...
For each touched stack, name the framework-reference skill(s) (e.g., `heroui-native`, `elysiajs`, `vercel-react-best-practices`) you'll consult during the four-question pre-flight and the implementation. **Multi-stack tasks** (a feature crossing apps/web + apps/native + apps/server, for example) consult refs for all touched stacks and produce ONE plan that spans them — do not split a coherent change into separate /ship runs.
...
**Announce the detected stack(s) and refs** in your initial response so the user can correct the assumption before the pre-flight begins.
...
4. Dispatch the **`reviewer-observability-coverage`** subagent (via `Agent(subagent_type=reviewer-observability-coverage)`) after critical-path async/error/integration changes; if it reports missing evidence, invoke `add-observability`.
...
11. Add or expand tests for backend logic, data transformations, permissions, contracts, persisted data, non-trivial branching, async behavior, and business rules — invoke `write-tests` so the new behavior is covered using the project's existing harness and conventions. In `production`, this step is mandatory; in `balanced`, run unless the change is trivial UI wiring; in `fast`, opt in only via `include=tests`.

## Stack-conditional adjuncts

- **UI wiring:** use `add-empty-error-states` for newly wired data that needs empty/error states.
- **Backend schema:** invoke `add-migration` before editing migrations for schema changes, then dispatch the **`reviewer-data-integrity`** subagent.
...
After the extension lands, run `simplify` against the diff to catch newly-introduced duplication, parallel-pattern drift, missed reuse of existing utilities, and parameter sprawl from grafting onto the existing shape.
