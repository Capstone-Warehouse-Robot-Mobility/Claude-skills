...
1. Read the routed skill's SKILL.md directly — for the core skills it's `.claude/skills/<skill-name>/SKILL.md` in this repo, or the equivalent path the host loads skills from.
...
Code is production-ready. To publish:
  ...
  • /open-mr          — open a GitLab MR (commits then pushes if needed)
```
...
- **NEVER commit, push, or open MRs from inside this skill**
  **Instead:** Stop at Step 5 and direct the user to `/commit`, `/commit-and-push`, or `/open-mr`.
...
- **UI scaffolding (when feature is user-facing):** `add-empty-error-states` (empty + error UI), `polish-ui` (post-step UX checklist), `propagate-ui-pattern` (when 3+ siblings of a recurring surface exist).
- **Backend scaffolding (when persisted data or schema changes):** `add-migration`, `add-observability` (integration-first lane), `audit-authz` (when the feature adds or changes server entry points with ownership/permission checks).
- **Tests (Phase 8):** `write-tests` (unit/integration).
- **Audits (Phase 7 gates):** reviewer-* subagents (contracts, concurrency, data-integrity, security-regression, error-boundaries, loading-states, accessibility-regression, client-bundle, observability-coverage, perf), audit-perf, audit-responsive.
- **Cleanup (post-step):** simplify, polish-ui.

### EVOLVE → `modify-feature` may invoke

- **UI extensions:** `add-empty-error-states`, `polish-ui`.
- **Backend extensions:** `add-migration`, `add-observability`, `audit-authz` (when the extension touches server entry points with ownership/permission checks).
- **Tests:** `write-tests`.
...
- **Regression pinning (after fix lands):** `add-regression-test`.
- **Polish if UI changed:** `polish-ui`.

### REMOVE → `remove-feature` may invoke

- **Schema cleanup:** `add-migration` (when removal drops columns/tables).
