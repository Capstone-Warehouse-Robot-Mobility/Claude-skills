...
Before scoping, identify which app(s) and package(s) the audit will cover by reading the user's prompt for explicit paths/scope hints (or `scope=<path>` if `/ship` passed one) and mapping to the monorepo layout — see `.claude/stack-map.md` for the full path → reference-skill mapping.

For each in-scope stack, name the framework-reference skill(s) (e.g., `heroui-native`, `elysiajs`, `vercel-react-best-practices`) the audit will consult when judging idiomaticity, performance, and convention adherence. **Whole-repo audits** consult refs for every stack present (web + native + server + auth + db). **Scoped audits** consult refs only for the in-scope stacks — don't load the elysiajs ref for a `apps/native/` audit.

The refs are reference docs (patterns, API surface, conventions) not workflows; consult via `Skill(skill="<name>")` or `Read` the file at `.claude/skills/<name>/SKILL.md`. The audit consults them during Phase 4 fan-out, not before — Phase 0 just announces which ones will be in play.

**Announce the detected stack(s) and refs** in your initial response so the user can correct the assumption before Phase 1 starts.
...
4. **Do not commit.** Hand off to the user: `/commit`, `/commit-and-push`, or `/open-mr`.
