...
| `production` | `balanced` + after Step 7 (propose fix), invoke `add-regression-test` to pin the bug with a test that fails on the pre-fix code and passes on the post-fix code. |
...
Before tracing, identify which app(s) and package(s) the failing integration crosses by reading the user's prompt for explicit paths/feature names and mapping to the monorepo layout — see `.claude/stack-map.md` for the full path → reference-skill mapping.

For each touched stack, name the framework-reference skill(s) (e.g., `heroui-native`, `elysiajs`, `vercel-react-best-practices`) you'll consult during runtime-contract tracing and patch design. **Multi-stack bugs** (a silent failure where the dispatch is in apps/server but the missing side effect is in apps/web, for example) consult refs for both sides — the trace itself often spans stacks.

The refs are reference docs (patterns, API surface, conventions) not workflows; consult via `Skill(skill="<name>")` or `Read` the file at `.claude/skills/<name>/SKILL.md`. Skip refs when the bug is clearly local (single-file logic error, off-by-one) and not at an integration seam.

**Announce the detected stack(s) and refs** in your initial response so the user can correct the assumption before tracing begins.
...
- **`reviewer-observability-coverage`** subagent (`Agent(subagent_type=reviewer-observability-coverage)`) — after critical-path async/error/integration fixes. If missing evidence is still in scope, invoke `add-observability`.
...
- `add-migration` — when the bug fix requires a corrective migration (schema needed an additional column the bug exposed, NOT NULL added without backfill, missing index causing the symptom, enum value never written, broken default, orphaned rows from a prior migration). Use this skill instead of hand-writing the migration so the fix gets the multi-phase deploy plan and data-integrity audit. Then dispatch the **`reviewer-data-integrity`** subagent against the migration + code diff.
- `realign` — when the root cause is not a local bug but a domain-model mismatch: enum/state/vocabulary drift, persisted status value mismatch, lifecycle-state rename, or a state machine whose code and business meaning diverged.

In `production`, also invoke `add-regression-test` unless explicitly skipped. If the patch touched UI files, run `polish-ui` after the checks above unless the UI delta is copy-only.
...
Once the bug is fixed and verified, run `simplify` on the diff to clean up any duplication, magic numbers, or quick-fix shortcuts the patch introduced — without changing behavior.
